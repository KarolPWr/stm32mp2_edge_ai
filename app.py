...
class GstWidget(Gtk.Box):
    ...
    def camera_pipeline_creation(self):
        """
        Creation of the pipeline using Gst.parse_launch.
        This forces the pipeline to match the working command line exactly.
        """
        # Define the pipeline as a string
        # 1. Source -> Convert -> Caps (Resolution) -> Tee
        cmd = "v4l2src device=/dev/" + str(self.app.video_device_prev) + " io-mode=2 ! "
        cmd += "videoconvert ! "
        cmd += "video/x-raw,width=" + str(args.frame_width) + ",height=" + str(args.frame_height) + " ! "
        cmd += "tee name=t "
        
        # 2. Branch 1: Display
        cmd += "t. ! queue leaky=2 max-size-buffers=1 ! videoconvert ! gtkwaylandsink name=gtk_sink "
        
        # 3. Branch 2: AI
        cmd += "t. ! queue leaky=2 max-size-buffers=1 ! videoconvert ! videoscale ! "
        cmd += "video/x-raw,format=RGB,width=" + str(self.app.nn_input_width) + ",height=" + str(self.app.nn_input_height) + " ! "
        cmd += "appsink name=app_sink emit-signals=true sync=false max-buffers=1 drop=true"

        print("GStreamer Pipeline String:", cmd)

        try:
            self.pipeline_preview = Gst.parse_launch(cmd)
        except Exception as e:
            print("Error creating pipeline:", e)
            return False

        self.gtkwaylandsink = self.pipeline_preview.get_by_name("gtk_sink")
        self.appsink = self.pipeline_preview.get_by_name("app_sink")

        if not self.gtkwaylandsink or not self.appsink:
            print("Error: Could not retrieve sinks from pipeline.")
            return False
...
class Application:
    ...
    def __init__(self, args):
        ...
        if args.image == "":
            ...
            # --- FIX: BYPASS BROKEN HELPER SCRIPT ---
            if (args.camera_src == "V4L2SRC"):
                # Directly use the user's argument (video7) instead of letting
                # setup_camera.sh auto-detect (incorrectly as video1)
                self.video_device_prev = args.video_device
                self.camera_caps_prev = ""
                self.dcmipp_sensor = ""
                self.main_postproc = ""
            # ----------------------------------------
            else:
                 # Original logic for LibCamera or other modes
